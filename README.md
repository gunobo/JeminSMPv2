...
| 🔥 킬 스트릭 보상 | 3·5·10·15·20 연속킬 시 💎 + XP 지급 (KDA 보정) |
| 💰 현상금 | `/bounty` |
| 💤 수면 투표 | `/sleepvote yes\|no` |
| 💀 사망 위치 | `/deathpoint` |
| 📍 웨이포인트 | `/wp add\|list\|tp\|del\|share` |
| 💤 AFK | 5분 무활동 → 탭에 `[AFK]` 표시 |
| 🔖 탭 목록 | `[Xms] [AFK] [칭호] 닉네임 💰X` 1초 갱신 |
| 📢 자동 공지 | 5분 주기 순환 방송 |
| 📈 플레이어 현황 | `/mystatus` |
| 🎒 스타터팩 | `/스타터팩` |
| 🌍 SMP 시작 | `/startSMP` |
...
```
/bounty <플레이어> <다이아>   — 현상금 걸기 (인벤에서 차감)
/bounty list                — 현상금 목록
/bounty cancel <플레이어>   — 내가 건 현상금 환불
```

- 복수 플레이어가 같은 대상에 중복 현상금 가능
- 킬러가 사살 시 합산 지급, 전체 방송
- 탭 목록에 `💰X` 표시
