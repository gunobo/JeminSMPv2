...
| ⛏ 직업/스킬 | `/job list\|info\|select`, `/skill use\|invest <heal\|deal\|force>` |
...
- 레벨업마다 스킬포인트 +1 (만렙 30), `/job info`에서 퀘스트 진행도 확인
- `/job select <직업>` — 최초 선택은 즉시 적용, 전직은 같은 명령어 한 번 더 입력해야 확정
- `/job buyscroll` — 💎 15개로 **전직의 서** 구매, 우클릭하면 확인 절차 없이 바로 직업 선택 GUI(채팅 버튼)가 뜸

### 스킬 (힐/딜/포스 역할은 공통, 효과는 직업마다 완전히 다름 — 레벨 1~3)
| 직업 | ✚ 힐 | ⚔ 딜 | ☠ 포스 |
|---|---|---|---|
| ⛏ 광부 | 대지의 축복 — 회복+채굴속도 버프 | 낙석 — 광역 피해+둔화 | 지진 — 광역 넉업+강한 슬로우+시야 뿌옇게(진탕) |
| 🌾 농부 | 수확의 축복 — 팀원까지 광역 회복 | 가시덩굴 — 광역 중독 | 뿌리 속박 — 강한 둔화+약화 |
| ⚔ 전사 | 전투 의지 — 자힐+저항 버프 | 돌격 — 전방 돌진하며 이동 경로의 적 전부 피해+넉백 | 포효 — 넓은 범위 약화 |
| 🎣 어부 | 만조 — 재생(지속 회복) | 작살 — 단일 대상 피해+끌어당김 | 파도 — 광역 강한 넉백 |

- 레벨 1→3 갈수록 수치/지속시간 증가, 쿨다운은 60→45→30초로 공통 단축
- `/skill invest <heal|deal|force>` — 스킬포인트 소모(레벨업마다 1/2/3 포인트, 최대 6포인트로 만렙)
- `/skill use <heal|deal|force>` — 액티브 스킬 사용 (파티클/사운드 이펙트 포함, 바닐라라 리소스팩 불필요)

### 리소스팩 (선택)
`resourcepack/` 폴더에 전직의 서 커스텀 아이템 모델 뼈대가 있음 (`CustomModelData 1001`). 텍스처(`textures/item/job_change_scroll.png`, 16×16)만 교체하면 실제 아이콘이 바뀜. 자세한 내용은 `resourcepack/README.md` 참고.
