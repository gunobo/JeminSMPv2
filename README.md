...
| 직업 | ✚ 힐 | ⚔ 딜 | ☠ 포스 | 🏃 이동기 |
|---|---|---|---|---|
| ⛏ 광부 | 대지의 축복 — 회복+채굴속도 버프 | 낙석 — 광역 피해+둔화 | 지진 — 광역 넉업+강한 슬로우+시야 뿌옇게(진탕) | 굴착 돌진 — 바라보는 방향 블록을 뚫으며 돌진 |
...
- **발동 방식**: 명령어가 아니라 **스킬템(막대기 기반 아이템)** 하나로 처리. 첫 스킬이 해금되면 자동 지급되며, 우클릭할 때마다 해금된 스킬을 순서대로 전환, 좌클릭/공격 시 현재 선택된 스킬이 발동. 사망 시 드롭되지 않고 부활하면 자동으로 다시 지급됨.
...
### 리소스팩 (선택)
`resourcepack/` 폴더에 커스텀 아이템 모델 뼈대가 있음: 전직의 서(`CustomModelData 1001`), 스킬템(직업×스킬 조합마다 다른 아이콘, `CustomModelData 2011~2044`, 16개). 텍스처만 교체하면 실제 아이콘이 바뀜. 자세한 내용은 `resourcepack/README.md` 참고.
