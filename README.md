...
### 1차 전직 (`/job advance`)
만렙(30) + 누적 조건 2개를 채우면 전직 가능. 전직하면 레벨 상한이 30 → 60으로 늘어나 계속 성장할 수 있고, 전용 칭호도 지급됨(자동 장착 안 되니 `/title equip`으로 장착). 레벨/스킬/패시브는 초기화되지 않고 그대로 유지됨. `/job info`에서 진행도 확인 가능.

| 직업 | 조건 1 (공통) | 조건 2 (직업별) |
|---|---|---|
| ⛏ 광부 | 누적 채굴 10,000회 | 다이아몬드/고대 잔해 500개 |
| 🌾 농부 | 누적 수확 10,000회 | 네더워트 1,000개 |
| ⚔ 전사 | 누적 처치 10,000회 | 플레이어 처치 100회 |
