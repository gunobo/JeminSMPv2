...
| 직업 | 경험치 획득 |
|---|---|
| ⛏ 광부 | 광물 채굴 (희귀할수록 경험치 ↑, 고대 잔해 최고) |
| 🌾 농부 | 다 자란 작물 수확 (밀/당근/감자/비트/네더워트) |
| ⚔ 전사 | 몹/플레이어 처치 (플레이어 킬이 더 높음) |
| 🎣 어부 | 낚시로 물고기 낚기 |
...
| 스킬 | 효과 (레벨 1 → 3) | 쿨다운 |
|---|---|---|
| ✚ 힐 | 즉시 체력 회복 6 → 10 → 14 | 60 → 45 → 30초 |
| ⚔ 딜 | 공격력 증가(신속) 5초/8초 → 8초(증폭 II) | 60 → 45 → 30초 |
| ☠ 포스 | 반경 4블록 둔화+약화+넉백 3초 → 4초 → 5초 | 60 → 45 → 30초 |

- `/skill invest <heal|deal|force>` — 스킬포인트 소모(레벨업마다 1/2/3 포인트, 최대 6포인트로 만렙)
- `/skill use <heal|deal|force>` — 액티브 스킬 사용
